...
router.post("/Product" ,async (req,res) => {
  ...
  router.get("/Product/getProductById/:id",async (req,res) => {
    try{
     const {id} = req.params ;
  ...
  router.get("/Product/Minus_Inventory/:id/:number",async(req,res) =>{
    try{
      const {id} = req.params ;
  ...
  router.get("/User/paid/:email" ,async (req, res) =>{
  try{
  ...
  router.get("/User/clear_shoppingCart/:email" ,async(req,res)=>{
    try{
     const {email} = req.params ;
     console.log(email)
  ...
  router.post("/User/addToCart" ,async(req , res)=> {
    ...
    try{
      ...
      //if exist , routerend , else , create one 
        try{
         // console.log(basket_product_id , productId);
           const result=   await pool.query(
            `update CUSTOMER  set  basket_product_id= ARRAY_routerEND( basket_product_id  ,${productId} ) where email='${email}'` ,
            [])
            console.log(result)
            res.send("added")
